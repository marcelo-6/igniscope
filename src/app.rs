...
use crate::archive::{
    compute_coverage, compute_project_counts, discover_resources_for_roots, inspect_archive,
...
pub fn run_summarize(archive_path: &Path, verbose: u8) -> Result<(), AppError> {
    ...
    println!(
        ...
        resource_inventories
            .iter()
            .map(|inventory| {
                let counts = compute_project_counts(&inventory.resources);
                let coverage = compute_coverage(&inventory.resources);
                (
                    inventory.project_root.clone(),
                    counts.resources_total,
                    coverage.unknown_resources,
                    coverage.unknown_ratio,
                )
            })
            .collect::<Vec<_>>()
    );
